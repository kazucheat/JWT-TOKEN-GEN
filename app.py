...
def extract_login_res(raw):
    ...
    while True:
        ...
        if parsed:
            return parsed
        idx += 1
    marker = raw.find(b"eyJhbGciOiJIUzI1NiIs")
    if marker != -1:
        for i in range(marker - 1, max(marker - 300, -1), -1):
            if raw[i] == 0x42:
...
def get_jwt_token():
    ...
    try:
        return jsonify(generate_jwt_token(uid, password)), 200
    except Exception as e:
        emit({
            "ts":         _now(),
